...
|---|---|---|---|
| `normal` | 正常模式 | 标准 | 全明 |
| `jieqi` | 揭棋 | 帅/将固定在本方九宫原位（明），其余 15 枚身份随机洗牌到本方其余 15 个初始格 | 15 枚暗子，**双方均不可见其身份**（客户端 `type: null`） |
| `chaosJieqi` | 全乱揭棋 | 本方 16 枚身份（含将/帅）随机洗牌到本方 16 个初始格 | 16 枚全暗，双方均不可见 |
| `chaosOpen` | 全明乱棋（"一开始就揭完的全乱揭棋"） | 同 `chaosJieqi` | 全明 |
